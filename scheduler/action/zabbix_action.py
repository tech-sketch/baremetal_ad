...
class ZabbixAction(object):
    ...
    def IpmiInterfaces(self):
        ...
        api.create_host_interface(self.host_name, '', ipaddress, 1, '623', 3, 1)
        api.set_ipmi_setting(self.host_name, -1, 4, "admin", "admin")
        api.attach_host_group(self.host_name, "OCP Servers")

        product_name = commands.getoutput('ipmitool -I lanplus -H ' + ipaddress + ' -U ' + 'admin' + ' -P ' + 'admin' + ' -A MD5 fru')
        product_name = commands.getoutput('echo "'+ product_name +'" | grep "Product Name" | cut -d: -f2 | tr -d " "')
    ...
    def AgentInterfaces(self):
        ...
        api.create_host_interface(self.host_name, '', ipaddress, 1, '10050', 1, 1)

        os_name = commands.getoutput('zabbix_get -s ' + ipaddress + ' -k system.sw.os | cut -d" " -f1')
